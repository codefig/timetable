...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", "E-Exam Timetable Generator"))
        ...
        self.actionGenerate_E_Exam_Timetable.setText(_translate("MainWindow", "Generate E-Exam Timetable"))
    ...
    def add_new_course(self):
        ...
        if self.db == None:
            ...
            insert_query = "INSERT INTO courses_cbt(code, title, no_of_students) VALUES(%s, %s, %s)"
            cursor = self.db.cursor()
            insert_tuple = (self.courseCode, self.courseTitle, self.courseNumberofStudent)
    ...
    def edit_course(self):
        ...
        if ok: 
            print("The username is : ", title)
            if(title == ''):
                QMessageBox.warning(self.newMainWindow, "Error ", "Course Title should not be empty")
            else:
                search_query = 'SELECT * FROM courses_cbt WHERE code = ' + "'"+str(title) + "'"
                print(search_query)
                cursor = self.db.cursor()
    ...
    def update_course(self, course_id, course_code, reg_students, course_title):
        update_query = "UPDATE courses_cbt SET code=%s, title=%s, no_of_students=%s WHERE id=%s" 
    ...
    def add_hall(self):
        self.hallui = Venue_Dialog()
        self.hallui.setupUi(self.hallDialog)
    
        self.hallui.hallDialog.accepted.connect(self.add_new_hall)
        self.hallui.hallDialog.rejected.connect(self.close_hall_dialog)
    ...
    def add_new_hall(self):
        ...
        self.hallName = self.hallui.hallNameInput.text()
        self.hallCapacity = self.hallui.hallCapacityInput.text()

        if self.db == None:
            QMessageBox.warning(self.newMainWindow, "Error ", "Please Connect to Database first")
        elif(self.hallName == '' or self.hallCapacity == ''):
            QMessageBox.about(self.newMainWindow, "Error", "Please Ensure Hall Name and Capacity are supplied")
        else:
            insert_query = "INSERT INTO halls_cbt(name, capacity) VALUES(%s, %s)"
            cursor = self.db.cursor()
            insert_tuple = (self.hallName, self.hallCapacity)
            result = cursor.execute(insert_query, insert_tuple)
            print("result is : ", result)
            if(cursor.lastrowid):
                QMessageBox.information(self.newMainWindow, "Hall ", "CBT Hall Added Successfully!")
                self.hallui.hallNameInput.setText("")
                self.hallui.hallCapacityInput.setText("")
            self.db.commit()
        ...
        if(self.db == None):
            ...
            if ok: 
                ...
                if(hallName == ''):
                    ...
                    search_query = 'SELECT * FROM halls_cbt WHERE name = ' + "'"+str(hallName) + "'"
                    print(search_query)
                    cursor = self.db.cursor()
                    cursor.execute(search_query)
                    row = cursor.fetchone()
                    if row is None:
                        ...
                        self.hallId = row[0]
                        self.hallui.hallNameInput.setText(str(row[1]))
                        self.hallui.hallCapacityInput.setText(str(row[2]))
                    
                
                        self.hallui.hallDialog.accepted.connect(lambda: self.update_hall(
                            self.hallId, 
                            self.hallui.hallNameInput.text(), 
                            self.hallui.hallCapacityInput.text()))
                        self.hallui.hallDialog.rejected.connect(self.hallDialog.reject)
                        self.hallDialog.exec_()
    
    def update_hall(self, id, name, capacity):
        print("this is the update hall funciton")
        update_query = "UPDATE halls_cbt SET name=%s,capacity=%s WHERE id=%s" 
        data =  (name, capacity, id)
        cursor = self.db.cursor()
        cursor.execute(update_query,data)
        self.db.commit()
    ...
    def view_halls(self):
        print("This is the view halls function")
        if self.db == None:
            QMessageBox.warning(self.newMainWindow, "Error ", "Please Connect to Database first")
        else:
            cursor = self.db.cursor()
            courses_query = "SELECT * FROM halls_cbt"
            cursor.execute(courses_query)
            rows = cursor.fetchall()
            print("Total rows : ", cursor.rowcount)
            output = ""
    ...
    def print_about(self):
        ...
        msg.setText(" CBT EXAM TIMETABLE GENERATOR")
        msg.setInformativeText("This Application generates the CBT/E-Exam Timetable")
